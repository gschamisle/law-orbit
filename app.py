...
from config import LAW_API_KEY, OPENAI_API_KEY, ENABLE_HWPX_OUTPUT, ENABLE_DRAFT_TAB, ENABLE_WIP_TABS, PUBLIC_GALAXY_ONLY
...
pages = [('galaxy', '법령은하', None)]
if not PUBLIC_GALAXY_ONLY:
    pages.append(('review', '개정안 검토 (국세)', amendment_review_ui))
if ENABLE_WIP_TABS and not PUBLIC_GALAXY_ONLY:
    pages += [('relations', '조문 연관 조회', article_relations_ui), ('new_article', '신설 조문 검토', new_article_ui)]
if ENABLE_DRAFT_TAB and not PUBLIC_GALAXY_ONLY:
    pages += [('draft', '조문안 작성', stage1_draft), ('crossref', '초안 인용 확인', stage2_crossref)]
if ENABLE_HWPX_OUTPUT and not PUBLIC_GALAXY_ONLY:
