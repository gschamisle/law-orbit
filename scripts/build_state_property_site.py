"""Extend a verified six-domain static release without rebuilding its original data."""
import argparse,hashlib,json,shutil
from pathlib import Path
from scripts.build_static_galaxies import Writer,read,shell,export_documents,overview,export_special_cases
from scripts.validate_static_galaxies import validate
from core.state_property_universe import BUNDLE,load_bundle,SECTORS
...
def build(base,destination,bundle_path=BUNDLE):
    ...
    if any(d['id']=='state_property' for d in manifest['domains']):raise ValueError('Base already contains national property')
    ...
    entries,ids=export_documents(writer,'state_property','',source['laws']+source['administrative_rules'],graph)
    catalog=dict(laws=entries,overview=writer.data(overview(graph,'state_property',ids,entries)),
                 coverage=graph['coverage_note'],built_at=graph['built_at'],sectors={'all':'전체 연결',**SECTORS},
                 special_cases=writer.data(export_special_cases(source['special_cases'],ids)))
    manifest['domains'].append(dict(id='state_property',title='국유재산',laws=len(entries),catalog=writer.data(catalog),built_at=graph['built_at']))
