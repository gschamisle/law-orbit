...
def render(law_api_key: str='', openai_api_key: str='') -> None:
    ...
    st.download_button('은하 내려받기',render_page(data).encode('utf-8'),f'금융법 은하-{SECTORS[sector]}.html','text/html',key=key('html'))
    if result:
        ...
        if sector!='all':
            ...
            with st.expander(f'분야 밖 관련 조문 · 공통·다른 업권 · {len(outside)}건',expanded=bool(outside)):
                st.caption('금융법 은하 안에서 수집·분석한 다른 분야의 조문입니다. 분야 태그는 개정 의무를 뜻하지 않습니다.')
                table(outside)
        ...
        st.download_button('금융 인용 근거 내려받기',json.dumps(dict(domain='fsc',sector=sector,selection=selection,rows=rows,
                           ...
                           '금융법 은하-인용근거.json','application/json',key=key('evidence'))
...
def _source_info(bundle: dict) -> None:
    ...
    with st.expander('금융법 은하 수집 범위와 분석 상태'):
